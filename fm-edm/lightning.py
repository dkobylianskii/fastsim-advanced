...
from models.model import FlowNetSimple, EDMNetSimple
from models.sampler_fm import pndm_sampler
from models.sampler_edm import pndm_sampler as pndm_sampler_edm
...
class FlowLightning(LightningModule):
    def __init__(self, config, comet_logger=None):
        ...
        self.train_class = config["train_class"]
        ...
        if self.model_type == "flow":
            self.net = FlowNetSimple(config)
            self.sigma = config["sigma"]
            self.FM = TargetConditionalFlowMatcher(sigma=self.sigma)
            self.time_pow = config.get("time_pow", False)
        elif self.model_type == "edm":
            self.net = EDMNetSimple(config)
            self.P_mean = float(config.get("p_mean", -0.8))
            self.P_std = float(config.get("p_std", 0.8))
            self.sigma_data = float(config.get("sigma_data", 1.1))
        else:
            raise NotImplementedError(
                f"Unknown model {self.model_type}, only 'flow' and 'edm' are supported"
            )
    ...
    def forward(self, data):
        ...
        if self.model_type == "flow":
            ...
            weight_bs = torch.ones(truth.shape[0], device=truth.device).reshape(
                -1, 1, 1
            )

        elif self.model_type == "edm":
            rnd_normal = torch.randn(truth.shape[0], device=truth.device)
            t = (rnd_normal * self.P_std + self.P_mean).exp()
            # lmbd = -2 * torch.log(t)
            weight = (t**2 + self.sigma_data**2) / (t * self.sigma_data) ** 2

            weight_bs = weight.reshape(-1, 1, 1)
            t_bs = t.reshape(-1, 1, 1)

            xt = pflow + torch.randn_like(pflow) * t_bs
            ut = pflow
        ...
        if pf_mask.sum() == 0:
            ...
            loss = weight_bs * self.loss(ut, vt)[pf_mask]

        return loss.mean(), loss_lik
    ...
    def log_image(self, truth, pflow, mask, scale, fs, fs_mask=None):
        ...
        if "sep" in self.model_type:
            truth = truth.flatten(1, 2)
            pflow = pflow.flatten(1, 2)
            fs = fs.flatten(1, 2)
            mask = mask.flatten(1, 2)
            if fs_mask is not None:
                fs_mask = fs_mask.flatten(1, 2)
