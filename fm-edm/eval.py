...
parser.add_argument("-m", "--method", type=str, default="pndm")
...
eval_path = f"{args.eval_dir}/{args.prefix}{config['name']}_{epoch}_{args.n_steps}_{args.method}_{'npf' if args.npf else ''}.root"
...
for i, (truth, pflow, mask, scale) in tqdm(enumerate(loader), total=len(loader)):
    ...
    if net.train_class:
        l_class_tr[n : truth.shape[0] + n] = (truth[..., 3] > 0.5).float().numpy()
    else:
    ...
    if net.train_class:
        l_class_pf[n : truth.shape[0] + n] = (pflow[..., 3] > 0.0).float().numpy()
    else:
    ...
    if net.train_class:
        l_class_fs[n : truth.shape[0] + n] = (fs[..., 3] > 0.0).float().numpy()
    else:
