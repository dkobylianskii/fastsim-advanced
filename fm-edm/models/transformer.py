...
class TransformerCrossAttentionLayer(nn.Module):
    ...
    def forward(
        ...
        "Pass through the layer using residual connections and layer normalisation"
        # Store the batch size, useful for reshaping
        b_size, seq, feat = q_seq.shape
        a_out = self.cross_attn(
            self.norm1(q_seq), self.norm0(kv_seq), kv_mask=kv_mask, q_mask=q_mask
        )

        if self.do_gru:
            a_out = self.gru(a_out.reshape(-1, feat), q_seq.reshape(-1, feat)).reshape(
                b_size, seq, feat
            )

        # q_seq = q_seq + a_out
        q_seq = q_seq + self.dense(self.norm2(a_out), ctxt)
